// MESSAGES

export const loadingMessage = 'We will take you there soon';
export const signInTitle = 'I already have an account';
export const signInDescription = 'Sign in with your email and password';
export const signUpTitle = 'I do not have an acoount';
export const signUpDescription = 'Sign up with your email and password';

// ERROR MESSAGES

export const invalidEmailError = 'Email is invalid';
export const requiredFieldError = 'Field is required';
export const passwordMatchError = 'Passwords don\'t match';
